...
int main(int argc, char* argv[]) {
	...
	mkdir("DATA/OUTPUT/pixdist-KMeans");
	mkdir("DATA/TEXT_COMPONENT/");
	mkdir("DATA/QUERY_COMPONENT/");
	ofstream log("DATA/OUTPUT/pixdist-KMeans/output.log");
		
	//Creating cluster
	cout<<"Creating kmeans cluster..."<<endl;
	KMeansTrainingSample sample("DATA/pixdist_feature.txt", 20);
	...
	for(vector<string>::iterator tfi=text_files.begin(); tfi!=text_files.end(); tfi++, text_num++) {
		...
		system(("feature_pixdist DATA\\TEXT_COMPONENT\\"+text_filename+" DATA\\TEXT_COMPONENT\\"+text_filename+".txt").data());
		...
		for(vector<string>::iterator qfi=query_files.begin(); qfi!=query_files.end(); qfi++, query_num++) {
			...
			if(text_num==0)
				system(("feature_pixdist DATA\\QUERY_COMPONENT\\"+query_filename+" DATA\\QUERY_COMPONENT\\"+query_filename+".txt").data());
			...
			double match_th=query.size()*0.06;
			...
			if(text_num==0)
				mkdir(("DATA/OUTPUT/pixdist-KMeans/"+query_filename).data());
			cvSaveImage(("DATA/OUTPUT/pixdist-KMeans/"+query_filename+"/"+text_filename+".jpg").data(), output_img);
			...
			if(text_num==0) {
				...
				cvSaveImage(("DATA/OUTPUT/pixdist-KMeans/"+query_filename+"/"+query_filename+".jpg").data(), query_img);
			}
