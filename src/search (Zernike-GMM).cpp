...
int main(int argc, char* argv[]) {
	mkdir("DATA/OUTPUT");
	mkdir("DATA/OUTPUT/ZERNIKE-GMM");
	mkdir("DATA/OUTPUT/ZERNIKE-GMM/TEXT_COMP");
	mkdir("DATA/OUTPUT/ZERNIKE-GMM/TEXT_FEATURE");
	mkdir("DATA/OUTPUT/ZERNIKE-GMM/QUERY_COMP");
	mkdir("DATA/OUTPUT/ZERNIKE-GMM/QUERY_FEATURE");
	mkdir("DATA/OUTPUT/ZERNIKE-GMM/OUTPUT");
	mkdir("DATA/OUTPUT/ZERNIKE-GMM/MATCH");
	...
	//Creating cluster
	GMMTrainingSample sample("DATA/zernike_feature.txt", 15);
	...
	for(vector<string>::iterator tfi=text_files.begin(); tfi!=text_files.end(); tfi++, text_num++) {
		...
		saveListComp(("DATA/OUTPUT/ZERNIKE-GMM/TEXT_COMP/"+text_filename).data(), text_img, text_comp);
	
		//Extracting Zernike feature of text
		system(("MATLAB -r \"zernike DATA\\OUTPUT\\ZERNIKE-GMM\\TEXT_COMP\\"+text_filename+"\\*.jpg DATA\\OUTPUT\\ZERNIKE-GMM\\TEXT_FEATURE\\"+text_filename+".txt\" -wait -nojvm -sd .").data());
		ImageFeature text(("DATA/OUTPUT/ZERNIKE-GMM/TEXT_FEATURE/"+text_filename+".txt").data());
		...
		for(vector<string>::iterator qfi=query_files.begin(); qfi!=query_files.end(); qfi++, query_num++) {
			...
			if(text_num==0) {
				...
				saveListComp(("DATA/OUTPUT/ZERNIKE-GMM/QUERY_COMP/"+query_filename).data(), query_img, query_comps[query_num]);
			}
			
			//Extracting Zernike feature of query
			if(text_num==0)
				system(("MATLAB -r \"zernike DATA\\OUTPUT\\ZERNIKE-GMM\\QUERY_COMP\\"+query_filename+"\\*.jpg DATA\\OUTPUT\\ZERNIKE-GMM\\QUERY_FEATURE\\"+query_filename+".txt\" -wait -nojvm -sd .").data());
			ImageFeature query(("DATA/OUTPUT/ZERNIKE-GMM/QUERY_FEATURE/"+query_filename+".txt").data());
			...
			for(MatchList::iterator i=asm_match.begin(); i!=asm_match.end(); i++) {
				...
				if(text_info_file==NULL)
					...
					ofstream output(("DATA/OUTPUT/ZERNIKE-GMM/MATCH/"+removeExtension(original_text_filename)+".txt").data(), ios::app);
			...
			if(text_num==0)
				mkdir(("DATA/OUTPUT/ZERNIKE-GMM/OUTPUT/"+query_filename).data());
			cvSaveImage(("DATA/OUTPUT/ZERNIKE-GMM/OUTPUT/"+query_filename+"/"+text_filename+".jpg").data(), output_img);
