...
int main(int argc, char* argv[]) {
	mkdir("DATA/OUTPUT/");
	mkdir("DATA/OUTPUT/400DIM-SOM");
	mkdir("DATA/OUTPUT/400DIM-SOM/TEXT_COMP");
	mkdir("DATA/OUTPUT/400DIM-SOM/TEXT_FEATURE");
	mkdir("DATA/OUTPUT/400DIM-SOM/QUERY_COMP");
	mkdir("DATA/OUTPUT/400DIM-SOM/QUERY_FEATURE");
	mkdir("DATA/OUTPUT/400DIM-SOM/OUTPUT");
	mkdir("DATA/OUTPUT/400DIM-SOM/MATCH");
	
	vector<string> text_files, query_files;
		
	//Creating cluster
	cout<<"Training SOM..."<<endl;
	system("som_pak\\randinit -din DATA\\400dim_feature.txt -cout DATA\\OUTPUT\\400DIM-SOM\\map.txt -xdim 4 -ydim 4 -topol rect -neigh gaussian -rand 123");
	system("som_pak\\vsom -din DATA\\400dim_feature.txt -cin DATA\\OUTPUT\\400DIM-SOM\\map.txt -cout DATA\\OUTPUT\\400DIM-SOM\\map.txt -rlen 1000 -alpha 0.05 -radius 10");
	system("som_pak\\vsom -din DATA\\400dim_feature.txt -cin DATA\\OUTPUT\\400DIM-SOM\\map.txt -cout DATA\\OUTPUT\\400DIM-SOM\\map.txt -rlen 10000 -alpha 0.02 -radius 3");
	system("som_pak\\qerror -din DATA\\400dim_feature.txt -cin DATA\\OUTPUT\\400DIM-SOM\\map.txt");
	system("som_pak\\vcal -din DATA\\400dim_feature.txt -cin DATA\\OUTPUT\\400DIM-SOM\\map.txt -cout DATA\\OUTPUT\\400DIM-SOM\\map.txt");
	SOM sample("DATA/OUTPUT/400DIM-SOM/map.txt");
	sample.visualize("DATA/OUTPUT/400DIM-SOM/som.jpg");
	...
	for(vector<string>::iterator tfi=text_files.begin(); tfi!=text_files.end(); tfi++, text_num++) {
		...
		saveListComp(("DATA/OUTPUT/400DIM-SOM/TEXT_COMP/"+text_filename).data(), text_img, text_comp);
	
		//Extracting feature of text
		system(("feature_400dim DATA\\OUTPUT\\400DIM-SOM\\TEXT_COMP\\"+text_filename+" DATA\\OUTPUT\\400DIM-SOM\\TEXT_FEATURE\\"+text_filename+".txt").data());
	
		//Creating Text Sequence
		ImageFeature text(("DATA/OUTPUT/400DIM-SOM/TEXT_FEATURE/"+text_filename+".txt").data());
		...
		for(vector<string>::iterator qfi=query_files.begin(); qfi!=query_files.end(); qfi++, query_num++) {
			...
			if(text_num==0) {
				...
				saveListComp(("DATA/OUTPUT/400DIM-SOM/QUERY_COMP/"+query_filename).data(), query_img, query_comps[query_num]);
			}
			
			//Extracting Zernike feature of query
			if(text_num==0)
				system(("feature_400dim DATA\\OUTPUT\\400DIM-SOM\\QUERY_COMP\\"+query_filename+" DATA\\OUTPUT\\400DIM-SOM\\QUERY_FEATURE\\"+query_filename+".txt").data());
			
			//Creating Query Sequence
			ImageFeature query(("DATA/OUTPUT/400DIM-SOM/QUERY_FEATURE/"+query_filename+".txt").data());
			...
			for(MatchList::iterator i=asm_match.begin(); i!=asm_match.end(); i++) {
				...
				if(text_info_file==NULL)
					...
					ofstream output(("DATA/OUTPUT/400DIM-SOM/MATCH/"+removeExtension(original_text_filename)+".txt").data(), ios::app);
			...
			if(text_num==0)
				mkdir(("DATA/OUTPUT/400DIM-SOM/OUTPUT/"+query_filename).data());
			cvSaveImage(("DATA/OUTPUT/400DIM-SOM/OUTPUT/"+query_filename+"/"+text_filename+".jpg").data(), output_img);
