...
int main(int argc, char* argv[]) {
	mkdir("DATA/OUTPUT/");
	mkdir("DATA/OUTPUT/DTW");
	mkdir("DATA/OUTPUT/DTW/TEXT_COMP/");
	mkdir("DATA/OUTPUT/DTW/TEXT_FEATURE/");
	mkdir("DATA/OUTPUT/DTW/TEXT_COMP/");
	mkdir("DATA/OUTPUT/DTW/QUERY_FEATURE/");
	mkdir("DATA/OUTPUT/DTW/OUTPUT/");
	mkdir("DATA/OUTPUT/DTW/MATCH/");
	...
	for(vector<string>::iterator tfi=text_files.begin(); tfi!=text_files.end(); tfi++, text_num++) {
		...
		saveListComp(("DATA/OUTPUT/DTW/TEXT_COMP/"+text_filename).data(), text_img, text_comp);
		...
		for(vector<string>::iterator qfi=query_files.begin(); qfi!=query_files.end(); qfi++, query_num++) {
			...
			if(text_num==0) {
				...
				saveListComp(("DATA/OUTPUT/DTW/QUERY_COMP/"+query_filename).data(), query_img, query_comps[query_num]);
			}
			
			//Searching Query in Text
			double **dist=new double*[query_comps[query_num].size()];
			for(int i=0; i<query_comps[query_num].size(); i++) {
				dist[i]=new double[text_comp.size()];
				IplImage* q=getCompImage(query_img, query_comps[query_num][i]);
				for(int j=0; j<text_comp.size(); j++) {
					IplImage* t=getCompImage(text_img, text_comp[j]);
					dist[i][j]=dTWDistance(q, t, 100, 100);
					cvReleaseImage(&t);
				}
				cvReleaseImage(&q);
			}
			double match_th=query_comps[query_num].size()*0.1;
			MatchList asm_match=approximateStringMatch(dist, text_comp.size(), query_comps[query_num].size(), match_th, false);
			...
			for(MatchList::iterator i=asm_match.begin(); i!=asm_match.end(); i++) {
				...
				if(text_info_file==NULL)
					...
					ofstream output(("DATA/OUTPUT/DTW/MATCH/"+removeExtension(original_text_filename)+".txt").data(), ios::app);
					output<<query_filename<<" "<<xgap+x1<<" "<<ygap+y1<<" "<<xgap+x2<<" "<<ygap+y2<<" "<<10*i->edit_distance/query_comps[query_num].size()<<endl;
					output.close();
			...
			if(text_num==0)
				mkdir(("DATA/OUTPUT/DTW/OUTPUT/"+query_filename).data());
			cvSaveImage(("DATA/OUTPUT/DTW/OUTPUT/"+query_filename+"/"+text_filename+".jpg").data(), output_img);
