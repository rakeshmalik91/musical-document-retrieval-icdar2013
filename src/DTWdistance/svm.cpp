...
#endif
template <class T> inline void swap_(T& x, T& y) { T t=x; x=y; y=t; }
...
void Cache::swap_index(int i, int j)
{
	...
	swap_(head[i].data,head[j].data);
	swap_(head[i].len,head[j].len);
...
void Solver::swap_index(int i, int j)
{
	...
	swap_(y[i],y[j]);
	swap_(G[i],G[j]);
	swap_(alpha_status[i],alpha_status[j]);
	swap_(alpha[i],alpha[j]);
	swap_(p[i],p[j]);
	swap_(active_set[i],active_set[j]);
	swap_(G_bar[i],G_bar[j]);
}
...
int **do_cross_validation(int nr_fold, const char *filename, int *data_size){
	svm_problem prob;	read_problem(filename, &prob);
	...
	*data_size = prob.l;
	int **Mat =  (int**)malloc(prob.l*sizeof(int*));
	for(int i = 0; i < prob.l; i++) Mat[i] = new int[2];
	...
	if(param.svm_type == EPSILON_SVR ||	   param.svm_type == NU_SVR){
		...
		for(i=0;i<prob.l;i++){
			...
			total_error += (v-y)*(v-y);
			sumv += v;	sumy += y;
			sumvv += v*v;	sumyy += y*y;	sumvy += v*y;
		}
		/*
		proc->filewrite<<"Cross Validation Mean squared error  "<<total_error/prob.l<<endl;
		proc->filewrite<<"Cross Validation Squared correlation coefficient  "<<
			((prob.l*sumvy-sumv*sumy)*(prob.l*sumvy-sumv*sumy))/
			((prob.l*sumvv-sumv*sumv)*(prob.l*sumyy-sumy*sumy))<<endl;
		*/
	}
	else{
		for(int i=0;i<prob.l;i++){
			Mat[i][0] = (int)prob.y[i];		Mat[i][1] = (int)target[i];
		}
	}
	free(target);
	return(Mat);
...
////svm_save_model("D:\\work\\data_out.txt",model);

svm_model *read_problem(const char *filename, svm_problem *OutProb){
	...
	while(1){
		...
		switch(c){
			case '\n':
				++prob.l;
				// fall through,	// count the '-1' element
			case ':':
				++elements;
				break;
			case EOF:
...
out2:
		if(j>=1 && x_space[j-1].index > max_index) 	max_index = x_space[j-1].index;
		x_space[j++].index = -1;
	}

	if(param.gamma == 0)	param.gamma = 1.0/max_index;
