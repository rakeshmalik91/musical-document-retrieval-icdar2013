...
int main(int argc, char* argv[]){
	...
	string outputDirName[] = {
		"DATA/OUTPUT/ZERNIKE-KMEANS/MATCH",//BLUE
		"DATA/OUTPUT/ZERNIKE-SOM/MATCH",//GREEN
		"DATA/OUTPUT/400DIM-KMEANS/MATCH",//RED
		"DATA/OUTPUT/400DIM-SOM/MATCH",//CYAN
		"DATA/OUTPUT/DTW/MATCH"//YELLOW
	};
	const char* csvFileName[] = {
		"DATA/OUTPUT/ZERNIKE-KMEANS.csv",
		"DATA/OUTPUT/ZERNIKE-SOM.csv",
		"DATA/OUTPUT/400DIM-KMEANS.csv",
		"DATA/OUTPUT/400DIM-SOM.csv",
		"DATA/OUTPUT/DTW.csv"
	};
	IplImage* graph=NULL;
	for(int iOutputDir = 0; iOutputDir < nOutputDir; iOutputDir++) {
		...
		closedir(outputDir);
		CvScalar color=(iOutputDir==0)?cvScalar(255):(iOutputDir==1)?cvScalar(0, 255):(iOutputDir==2)?cvScalar(0, 0, 255):(iOutputDir==3)?cvScalar(255, 255):cvScalar(0, 255, 255);
		graph=curve.plot(csvFile, graph, 2, color);
		csvFile.close();
	}
	cvSaveImage("DATA/OUTPUT/curve.jpg", graph);
