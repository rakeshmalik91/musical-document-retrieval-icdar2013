...
int main(int argc, char* argv[]) {
	...
	mkdir("DATA/OUTPUT/400DIM-KMEANS/TEXT_COMP");
	mkdir("DATA/OUTPUT/400DIM-KMEANS/TEXT_FEATURE");
	mkdir("DATA/OUTPUT/400DIM-KMEANS/QUERY_COMP");
	mkdir("DATA/OUTPUT/400DIM-KMEANS/QUERY_FEATURE");
	mkdir("DATA/OUTPUT/400DIM-KMEANS/OUTPUT");
	mkdir("DATA/OUTPUT/400DIM-KMEANS/MATCH");
	...
	//Creating cluster
	KMeansTrainingSample sample("DATA/400dim_feature.txt", 25);
	...
	for(vector<string>::iterator tfi=text_files.begin(); tfi!=text_files.end(); tfi++, text_num++) {
		...
		saveListComp(("DATA/OUTPUT/400DIM-KMEANS/TEXT_COMP/"+text_filename).data(), text_img, text_comp);
	
		//Extracting feature of text
		system(("feature_400dim DATA\\OUTPUT\\400DIM-KMEANS\\TEXT_COMP\\"+text_filename+" DATA\\OUTPUT\\400DIM-KMEANS\\TEXT_FEATURE\\"+text_filename+".txt").data());
		ImageFeature text(("DATA/OUTPUT/400DIM-KMEANS/TEXT_FEATURE/"+text_filename+".txt").data());
		...
		for(vector<string>::iterator qfi=query_files.begin(); qfi!=query_files.end(); qfi++, query_num++) {
			...
			if(text_num==0) {
				...
				saveListComp(("DATA/OUTPUT/400DIM-KMEANS/QUERY_COMP/"+query_filename).data(), query_img, query_comps[query_num]);
			}
			
			//Extracting feature of query
			if(text_num==0)
				system(("feature_400dim DATA\\OUTPUT\\400DIM-KMEANS\\QUERY_COMP\\"+query_filename+" DATA\\OUTPUT\\400DIM-KMEANS\\QUERY_FEATURE\\"+query_filename+".txt").data());
			ImageFeature query(("DATA/OUTPUT/400DIM-KMEANS/QUERY_FEATURE/"+query_filename+".txt").data());
			...
			for(MatchList::iterator i=asm_match.begin(); i!=asm_match.end(); i++) {
				...
				if(text_info_file==NULL)
					...
					ofstream output(("DATA/OUTPUT/400DIM-KMEANS/MATCH/"+removeExtension(original_text_filename)+".txt").data(), ios::app);
			...
			if(text_num==0)
				mkdir(("DATA/OUTPUT/400DIM-KMEANS/OUTPUT/"+query_filename).data());
			cvSaveImage(("DATA/OUTPUT/400DIM-KMEANS/OUTPUT/"+query_filename+"/"+text_filename+".jpg").data(), output_img);
