...
namespace image_processing {
	...
	typedef vector<MInt> ListInt;
	typedef struct MPix{
		CvPoint  V;
		int      VI;
		double	VD;
	}MPix;
	typedef vector<MPix> ListPix;
	...
	void compFilter(ListComp &comp_list, int MIN_WIDTH, int MIN_HEIGHT=0, double MIN_HEIGHT_WIDTH_RATIO=INFINITY, double MIN_WIDTH_HEIGHT_RATIO=INFINITY) {
		for(ListComp::iterator i=comp_list.begin(); i!=comp_list.end();) {
			...
			if(width>=MIN_WIDTH && height>=MIN_HEIGHT && height/(double)width<=MIN_HEIGHT_WIDTH_RATIO && width/(double)height<=MIN_WIDTH_HEIGHT_RATIO)
				i++;
			else
				i=comp_list.erase(i);
		}
	...
	ListComp getComponents(IplImage* img) {
		...
		compFilter(comp_list, 10, 20, 7, 5);
	...
	void saveListComp(const char *dirname, IplImage* img, ListComp comp_list) {
		mkdir(dirname);
		for(int i=0; i<comp_list.size(); i++) {
			char out_name[100];
			sprintf(out_name, "%s/%.5d.jpg", dirname, i);
			saveComp(out_name, img, comp_list[i]);
		}
	}
	...
	ListPix* ContourPixelSet(IplImage *src, bool flag, int *N){
		CvMemStorage* storage = cvCreateMemStorage(0);
		CvSeq*	contour = ContourSeq( src, flag, storage);
		MPix P;
		int NList = 0;
		for(CvSeq *contourtemp = contour  ; contourtemp != 0; contourtemp = contourtemp->h_next )	NList++;
		*N = NList;
	
		ListPix *LP = new ListPix[NList];
		int set = 0;
		for(CvSeq *contourtemp = contour ; contourtemp != 0; contourtemp = contourtemp->h_next ){
			for( int i = 0; i < contourtemp->total; i++){
				CvPoint *CP = (CvPoint*)cvGetSeqElem( contourtemp, i );
