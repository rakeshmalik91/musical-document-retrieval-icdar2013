   ...
   environment variable. 
	...
	A. Locate the name of the environment variable to set, using the 
	 ...
	 NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_TestDir.sh on UNIX and Mac
...
3. Create packages that contain the following files, based on what
   you are deploying: 
	...
	All Platforms:
		* TestDir.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
	Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library archive)  
		* TestDir.exe (application created by MATLAB Compiler)
	UNIX:
		* MCRInstaller.zip (MATLAB Component Runtime library 
	 	  archive - manual unzip required)
		* TestDir (application created by MATLAB Compiler)
		* run_TestDir.sh (shell script that can be run 
	...
	All Platforms:            
		* TestDir.dll (shared library, file extension 
		  varies by platform)
		* TestDir.h (library header file)
		* TestDir.ctf (Component Technology File archive,
		  platform-dependent on target machine)
	...
	* TestDir.ctf (Component Technology File archive)	
	* TestDir.xml (documentation files)
	* TestDir.pdb (program database file - if 
	  DEBUG option selected)
	* TestDir.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* TestDir.ctf (Component Technology File archive)
	* TestDir_1_0.dll (component containing 
	  compiled M-code)
	...
	* TestDir.jar (Java archive containing Java 
	  interface to M-code in TestDir.ctf)
	* TestDir.ctf (Component Technology File archive,
	  platform-dependent on target machine)
