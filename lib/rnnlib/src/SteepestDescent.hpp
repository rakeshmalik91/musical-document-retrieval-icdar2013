...
struct SteepestDescent: public DataExporter, public Optimiser
{
	...
	//functions
	SteepestDescent(ostream& o, vector<real_t>& weights, vector<real_t>& derivatives, 
			vector<real_t>* plasticities = 0, real_t lr = 1e-4, real_t mom = 0.9, 
			const string& name = "optimiser"):
		DataExporter(name),
		Optimiser(weights, derivatives, plasticities),
		out(o),
		learnRate(lr),
		momentum(mom)
	{
	...
	void update_weights()
	{
		...
		if (plasts)
		{
			...
			{
				real_t delta = (*plasts)[i] * ((momentum * deltas[i]) - (learnRate * derivs[i]));
				deltas[i] = delta;
				wts[i] += delta;
			}
		}
