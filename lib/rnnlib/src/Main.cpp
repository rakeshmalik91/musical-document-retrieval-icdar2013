...
int main(int argc, char* argv[])
{
	...
	{
		opt = new Rprop(out, wc.weights, wc.derivatives, &wc.plasticities);
	}
	else
	{
		real_t learnRate = conf.get<real_t>("learnRate", 1e-4);
		real_t momentum = conf.get<real_t>("momentum", 0.9);
		opt = new SteepestDescent(out, wc.weights, wc.derivatives, &wc.plasticities, learnRate, momentum);
	}
	...
	if (numRandWts)
	{
		out << numRandWts << " uninitialised weights randomised with mean 0 std. dev. " << initWeightRange << endl << endl;
	}
