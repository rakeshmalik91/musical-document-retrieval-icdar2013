...
int main(int argc, char* argv[]) {
	...
	ofstream log("DATA/OUTPUT/Zernike-GMM/output.log");
		
	//Creating cluster
	cout<<"Creating kmeans cluster..."<<endl;
	GMMTrainingSample sample("DATA/zernike_feature.txt", 20);
	...
	for(vector<string>::iterator tfi=text_files.begin(); tfi!=text_files.end(); tfi++, text_num++) {
		...
		system(("MOVE feature.txt DATA\\TEXT_COMPONENT\\"+text_filename+".txt").data());
		...
		for(vector<string>::iterator qfi=query_files.begin(); qfi!=query_files.end(); qfi++, query_num++) {
			...
			if(text_num==0) {
				...
				system(("MOVE feature.txt DATA\\QUERY_COMPONENT\\"+query_filename+".txt").data());
			}
			...
			if(text_num==0)
				mkdir(("DATA/OUTPUT/Zernike-GMM/"+query_filename).data());
			cvSaveImage(("DATA/OUTPUT/Zernike-GMM/"+query_filename+"/"+text_filename+".jpg").data(), output_img);
			...
			if(text_num==0) {
				...
				cvSaveImage(("DATA/OUTPUT/Zernike-GMM/"+query_filename+"/"+query_filename+".jpg").data(), query_img);
			}
