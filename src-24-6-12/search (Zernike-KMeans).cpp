...
int main(int argc, char* argv[]) {
	...
	ofstream log("DATA/OUTPUT/Zernike-KMeans/output.log");
	...
	for(vector<string>::iterator tfi=text_files.begin(); tfi!=text_files.end(); tfi++, text_num++) {
		...
		for(vector<string>::iterator qfi=query_files.begin(); qfi!=query_files.end(); qfi++, query_num++) {
			...
			if(text_num==0)
				mkdir(("DATA/OUTPUT/Zernike-KMeans/"+query_filename).data());
			cvSaveImage(("DATA/OUTPUT/Zernike-KMeans/"+query_filename+"/"+text_filename+".jpg").data(), output_img);
			...
			if(text_num==0) {
				...
				cvSaveImage(("DATA/OUTPUT/Zernike-KMeans/"+query_filename+"/"+query_filename+".jpg").data(), query_img);
			}
