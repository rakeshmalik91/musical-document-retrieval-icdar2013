...
int main(int argc, char* argv[]) {
	...
	ofstream log("DATA/OUTPUT/Zernike-SOM/output.log");
		
	//Creating cluster
	cout<<"Training SOM..."<<endl;
	system("som_pak\\randinit -din DATA\\zernike_feature.txt -cout DATA\\OUTPUT\\Zernike-SOM\\map.txt -xdim 5 -ydim 5 -topol rect -neigh gaussian -rand 123");
	...
	for(vector<string>::iterator tfi=text_files.begin(); tfi!=text_files.end(); tfi++, text_num++) {
		...
		system(("zernike\\main DATA\\TEXT_COMPONENT\\"+text_filename+"\\*.jpg").data());
		system(("MOVE zernike_feature.txt DATA\\TEXT_COMPONENT\\"+text_filename+".txt").data());
		...
		for(vector<string>::iterator qfi=query_files.begin(); qfi!=query_files.end(); qfi++, query_num++) {
			...
			if(text_num==0) {
				system(("zernike\\main DATA\\QUERY_COMPONENT\\"+query_filename+"\\*.jpg").data());
				system(("MOVE zernike_feature.txt DATA\\QUERY_COMPONENT\\"+query_filename+".txt").data());
			}
			...
			double match_th=query.size()*0.2;
			...
			if(text_num==0)
				mkdir(("DATA/OUTPUT/Zernike-SOM/"+query_filename).data());
			cvSaveImage(("DATA/OUTPUT/Zernike-SOM/"+query_filename+"/"+text_filename+".jpg").data(), output_img);
			...
			if(text_num==0) {
				...
				cvSaveImage(("DATA/OUTPUT/Zernike-SOM/"+query_filename+"/"+query_filename+".jpg").data(), query_img);
			}
